...
#' @export
checkReportFolder= function(path){
...
getNoTempReport= function(file_path){
  ...
  for(i in 1:dim(tmp_log)[1]){
    if(!tmp_log$Employee[i] %in% exempt_list)
...
getNoQuesReport= function(file_path){
  ...
  for(i in 1:dim(quest_log)[1]){
    if(!quest_log$Employee[i]%in% exempt_list)
...
getNoContactReport= function(file_path){
  report_log <- readxl::read_excel(paste("C:/Users/ndeff/Downloads/covid-details-",Sys.Date(),".xlsx",sep = ""), sheet = "Last Report Dates")
  ...
  for(i in 1:dim(report_log)[1]){
    if(!report_log$Employee[i]%in% exempt_list){
...
getOverdueReport= function(file_path){
  tests_log <- readxl::read_excel(paste("C:/Users/ndeff/Downloads/covid-details-",Sys.Date(),".xlsx",sep = ""), sheet = "COVID Tests")
  ...
  for(i in 1:dim(tests_log)[1]){
    if(!tests_log$Employee[i]%in% exempt_list){
